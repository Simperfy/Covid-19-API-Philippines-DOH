...
class DatabaseAdapter {
   ...
   */
  connect = async (database: any) => 'Mocked Database Connected Successfully';
  ...
  get = async (queries: any) => {
    ...
    for (let i = 0; i < queries.limit; i += 1) {
      res.push({
   ...
   * @return {Promise}
   */
  getFacilitiesSummary = async (queries: any) => ({
   ...
   */
  getLatestFolderID = async () => 'Mocked Folder ID';

  /**
   *
   * @return {Promise<String>}
   */
  getLastUpdateDate = async () => '8/5/2020, 10:10:33 PM';
   ...
   * @return {Promise<void>}
   */
  truncate = async (tableName: string) => null;
   ...
   * @return {Promise<String>}
   */
  insert = async (tableName: string, fieldValueObj: any) => 'Mocked Inserted';
   ...
   */
  updateDatabaseFromCSV = async () => true;
}
